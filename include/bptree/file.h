...
class FileHandlerImpl<OS::LINUX> {
 public:
   ...
   */
  static FileHandlerImpl OpenFile(const std::string& filename, bool sync = false) {
   ...
   */
  static FileHandlerImpl CreateFile(const std::string& filename, bool sync = false) {
    int flag = O_RDWR | O_DIRECT | O_CREAT | O_EXCL;
    if (sync == true) {
      flag |= O_SYNC;
    }
    int fd = open(filename.c_str(), flag, 0666);
